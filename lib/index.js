...
exports.default = (filesData, root, extensions) => {
    const result = Object.entries(filesData).reduce((acc, [filePath, data]) => {
        ...
        const dependencies = imports.map(i => extract_path_1.default(i, filePath, root, extensions));
...
exports.default = (ast) => {
    ...
    traverse_1.default(ast, {
        enter(path) {
            const isStaticImport = path.node.type === 'ImportDeclaration' ||
                ((path.node.type === 'ExportNamedDeclaration' ||
                    path.node.type === 'ExportAllDeclaration') &&
                    path.node.source);
            const isDynamicImport = path.node.type === 'CallExpression' &&
